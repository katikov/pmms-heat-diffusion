...
void do_compute(const struct parameters* p, struct results *r)
{
    int N = p->N;
    int M = p->M;
    //int num_cols = ((M+3)/4)*4+2;  // M/4 upper_bound
    int num_cols = (M+1)/2*2+2;
    ...
    for(int iter=0; iter<p->maxiter; iter++){
        ...
        for(int i=1;i<=N;i++){
            ...
            for(int j=1;j<=M;j+=2){
                ...
                double d4 = cur_world[(i-1)*(M+2)+j+3] + cur_world[(i+1)*(M+2)+j+3];
                double d5 = cur_world[(i-1)*(M+2)+j+4] + cur_world[(i+1)*(M+2)+j+4];
                ...
                double temp1 = cur_world[i*(M+2)+j+1] * conductivity[(i-1)*M+j]
                    ...
                    ;
                double temp2 = cur_world[i*(M+2)+j+2] * conductivity[(i-1)*M+j+1]
                    +    (d2+d4)*weight_diag[(i-1)*M+j+1]
                    +   (d3 + cur_world[i*(M+2)+j+1] + cur_world[i*(M+2)+j+3])*weight_direct[(i-1)*M+j+1]
                    ;
                double temp3 = cur_world[i*(M+2)+j+3] * conductivity[(i-1)*M+j+2]
                    +    (d3+d5)*weight_diag[(i-1)*M+j+2]
                    +   (d4 + cur_world[i*(M+2)+j+2] + cur_world[i*(M+2)+j+4])*weight_direct[(i-1)*M+j+2]
                    ;
                d0 = d4;
                d1 = d5;
                ...
                next_world[i*(M+2)+j+2] = temp2;
                next_world[i*(M+2)+j+3] = temp3;
            */
           /*
               ...
               /*
                ...
                __m256d up = _mm256_loadu_pd(&cur_world[(i-1)*num_cols+j]);
                __m256d down = _mm256_loadu_pd(&cur_world[(i+1)*num_cols+j]);
                __m256d left = _mm256_loadu_pd(&cur_world[i*num_cols+j-1]);
                __m256d right = _mm256_loadu_pd(&cur_world[i*num_cols+j+1]);
                __m256d sum_ud = _mm256_add_pd(up,down);
                __m256d sum_lr = _mm256_add_pd(left,right);
                __m256d sum_direct = _mm256_add_pd(sum_lr, sum_ud);
                sum = _mm256_fmadd_pd(sum_direct, _mm256_loadu_pd(&weight_direct[(i-1)*M+j-1]),sum);


                __m256d ul = _mm256_loadu_pd(&cur_world[(i-1)*num_cols+j-1]);
                __m256d ur = _mm256_loadu_pd(&cur_world[(i-1)*num_cols+j+1]);
                __m256d dl = _mm256_loadu_pd(&cur_world[(i+1)*num_cols+j-1]);
                __m256d dr = _mm256_loadu_pd(&cur_world[(i+1)*num_cols+j+1]);
                __m256d sum_u = _mm256_add_pd(ul,ur);
                __m256d sum_d = _mm256_add_pd(dl,dr);
                __m256d sum_diag = _mm256_add_pd(sum_u, sum_d);
                sum = _mm256_fmadd_pd(sum_diag, _mm256_loadu_pd(&weight_diag[(i-1)*M+j-1]),sum);
               ...
               /*
                ...
                __m128d up = _mm_loadu_pd(&cur_world[(i-1)*num_cols+j]);
                __m128d down = _mm_loadu_pd(&cur_world[(i+1)*num_cols+j]);
                __m128d left = _mm_loadu_pd(&cur_world[i*num_cols+j-1]);
                __m128d right = _mm_loadu_pd(&cur_world[i*num_cols+j+1]);
                __m128d sum_ud = _mm_add_pd(up,down);
                __m128d sum_lr = _mm_add_pd(left,right);
                __m128d sum_direct = _mm_add_pd(sum_lr, sum_ud);
                sum = _mm_fmadd_pd(sum_direct, _mm_loadu_pd(&weight_direct[(i-1)*M+j-1]),sum);


                __m128d ul = _mm_loadu_pd(&cur_world[(i-1)*num_cols+j-1]);
                __m128d ur = _mm_loadu_pd(&cur_world[(i-1)*num_cols+j+1]);
                __m128d dl = _mm_loadu_pd(&cur_world[(i+1)*num_cols+j-1]);
                __m128d dr = _mm_loadu_pd(&cur_world[(i+1)*num_cols+j+1]);
                __m128d sum_u = _mm_add_pd(ul,ur);
                __m128d sum_d = _mm_add_pd(dl,dr);
                __m128d sum_diag = _mm_add_pd(sum_u, sum_d);
