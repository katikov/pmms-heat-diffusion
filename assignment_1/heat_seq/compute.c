...
void do_compute(const struct parameters* p, struct results *r)
{
    ...
    // for(int i=0;i<p->N;i++){
    //     for(int j=0;j<p->M;j++){
    //         cur_world[j*(p->N+2) + i + 1] = p->tinit[i*p->M + j];              
    //     }
    // }
    ...
    for(int iter=0; iter<p->maxiter; iter++){
        ...
        for(int i=1;i<=N;i++){
            for(int j=1;j<=M;j++){
                next_world[i*(M+2)+j] = cur_world[i*(M+2)+j] * conductivity[(i-1)*M+j-1]
                    + (     (cur_world[(i-1)*(M+2)+j-1] + cur_world[(i-1)*(M+2)+j+1] 
                            + cur_world[(i+1)*(M+2)+j-1] + cur_world[(i+1)*(M+2)+j+1])*weight_diag
                        +   (cur_world[(i-1)*(M+2)+j] + cur_world[i*(M+2)+j-1]
                            + cur_world[i*(M+2)+j+1] + cur_world[(i+1)*(M+2)+j])*weight_direct
                    ) 
                    * (1-conductivity[(i-1)*M+j-1]);
            }
        }
